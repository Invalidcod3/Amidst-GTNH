...
final class SpaceStructurePredictor {
    ...
    private static final int AMUN_RA_REGION_SIZE_BLOCKS = 32 * BLOCKS_PER_CHUNK;
    private static final long ANUBIS_VILLAGE_SALT = 1098540180186541L;
    private static final long HORUS_PYRAMID_SALT = 549865610521L;
    ...
    List<RoguelikeDungeonPredictor.StructureDescriptor> predict(
        ...
        if (matches(
                ...
                dimension,
                SpaceDimensionSampler.marsDimensionId())) {
            ...
            addDungeons(
                ...
                dimension,
                SpaceDimensionSampler.asteroidsDimensionId())) {
            ...
            addAbandonedBases(
                ...
                "galaxyspace:ceres",
                dimension,
                SpaceDimensionSampler.ceresDimensionId())) {
            addDungeons(
                    seed,
                    dimension,
                    "CERES_DUNGEON",
                ...
                "galaxyspace:io",
                dimension,
                SpaceDimensionSampler.ioDimensionId())) {
            addDungeons(
                    seed,
                    dimension,
                    "IO_DUNGEON",
                ...
                dimension,
                SpaceDimensionSampler.enceladusDimensionId())) {
            addDungeons(
                    seed,
                    dimension,
                    "ENCELADUS_DUNGEON",
                ...
                "galaxyspace:proteus",
                dimension,
                SpaceDimensionSampler.proteusDimensionId())) {
            addDungeons(
                    seed,
                    dimension,
                    "PROTEUS_DUNGEON",
                ...
                "galaxyspace:pluto",
                dimension,
                SpaceDimensionSampler.plutoDimensionId())) {
            addDungeons(
                    seed,
                    dimension,
                    "PLUTO_DUNGEON",
                ...
                dimension,
                SpaceDimensionSampler.mehenBeltDimensionId())) {
            ...
            addRoss128bRuins(seed, minX, minZ, maxX, maxZ, result);
                ...
                dimension,
                SpaceDimensionSampler.deepDarkDimensionId())) {
            addDeepDarkDungeons(seed, minX, minZ, maxX, maxZ, result);
        } else if (matches(
                dimensionKey,
                "amunra:anubis",
                dimension,
                SpaceDimensionSampler.anubisDimensionId())) {
            addAmunRaStructures(
                    seed,
                ...
                "amunra:horus",
                dimension,
                SpaceDimensionSampler.horusDimensionId())) {
            addAmunRaStructures(
                    seed,
                ...
                dimension,
                SpaceDimensionSampler.barnardaCDimensionId())) {
            // Barnarda C currently has no requested seed-locatable structure.
        } else {
    ...
    private static boolean matches(
        ...
        return SpaceDimensionSampler.matches(
                dimensionKey,
                expectedKey,
                dimension,
                expectedDimension);
    ...
    /**
     * Extra Utilities places one WorldGenCastle in every 512x512 block region.
     * WorldGenCastle#castleX/castleZ use a dedicated seed-only Random, so the
     * location is independent of chunk population order.
     */
    private static void addDeepDarkDungeons(
            long seed,
            int minX,
            int minZ,
            long maxX,
            long maxZ,
            List<RoguelikeDungeonPredictor.StructureDescriptor> result) {
        RegionBounds regions = blockRegions(
                minX,
                minZ,
                maxX,
                maxZ,
                AMUN_RA_REGION_SIZE_BLOCKS);
        validateCount(regions, "Deep Dark dungeon regions");
        Random random = new Random();
        for (int regionZ = regions.minZ; regionZ <= regions.maxZ; regionZ++) {
            for (int regionX = regions.minX; regionX <= regions.maxX; regionX++) {
                random.setSeed(seed + regionX + 65535L * regionZ);
                int x = regionX * AMUN_RA_REGION_SIZE_BLOCKS + random.nextInt(390) + 61;
                int z = regionZ * AMUN_RA_REGION_SIZE_BLOCKS + random.nextInt(390) + 61;
                addIfInside(
                        "DEEP_DARK_DUNGEON",
                        "",
                        x,
    ...
    /**
     * Amun-Ra GridVillageGenerator and PyramidGenerator share the same
     * one-candidate-per-32x32-chunk layout, differing only in their salt.
     */
    private static void addAmunRaStructures(
            long seed,
            String kind,
            long salt,
            boolean randomBlockOffset,
            int minX,
            int minZ,
            long maxX,
            long maxZ,
        ...
        RegionBounds regions = blockRegions(
                ...
                maxZ,
                AMUN_RA_REGION_SIZE_BLOCKS);
        validateCount(regions, "Amun-Ra structure regions");
        for (int regionZ = regions.minZ; regionZ <= regions.maxZ; regionZ++) {
            for (int regionX = regions.minX; regionX <= regions.maxX; regionX++) {
                Random candidate = new Random(seed ^ salt ^ regionX ^ regionZ);
                int chunkX = regionX * 32 + candidate.nextInt(32);
                int chunkZ = regionZ * 32 + candidate.nextInt(32);
                int x = chunkX * BLOCKS_PER_CHUNK;
                int z = chunkZ * BLOCKS_PER_CHUNK;
                if (randomBlockOffset) {
                    Random structure = new Random(seed ^ salt ^ chunkX ^ chunkZ);
                    x += structure.nextInt(BLOCKS_PER_CHUNK);
                    z += structure.nextInt(BLOCKS_PER_CHUNK);
                }
                addIfInside(
                        kind,
                        "",
    ...
    private static RegionBounds blockRegions(
            int minX,
            int minZ,
            long maxX,
            long maxZ,
            int regionSize) {
        return new RegionBounds(
                checkedInt(Math.floorDiv((long) minX, regionSize)),
                checkedInt(Math.floorDiv(maxX - 1L, regionSize)),
                checkedInt(Math.floorDiv((long) minZ, regionSize)),
                checkedInt(Math.floorDiv(maxZ - 1L, regionSize)));
    }
