...
final class SurfaceBiomeSamplers {
    ...
    private static final class RwgSurfaceBiomeSampler implements SurfaceBiomeSampler {
        ...
        private RwgSpawnCaves spawnCaves;
        private final Map<Long, SpawnSearch.Surface[]> spawnSurfaces =
                new LinkedHashMap<Long, SpawnSearch.Surface[]>(32, 0.75F, true) {
                    @Override protected boolean removeEldestEntry(Map.Entry<Long, SpawnSearch.Surface[]> e) {
                        return size() > 128;
                    }
                };
        ...
        private BiomeGenBase[] replayChunk(int chunkX, int chunkZ, BlendChunk chunk)
            ...
            BiomeGenBase[] result = replay.replay(chunkX, chunkZ, realistic, base, rivers, heights, rawRivers, mapBiomes.toArray());
            return result;
        }
        ...
        public SpawnSearch.Surface getSpawnSurface(int x, int z) {
            ...
            SpawnSearch.Surface[] surfaces = spawnSurfaces.get(key);
            if (surfaces == null) {
                try {
                    // Biome-only cache hits do not contain terrain. Always replay the
                    // entire chunk before reading a spawn column, including surface callbacks.
                    if (spawnCaves == null) spawnCaves = RwgSpawnCaves.create(previewWorld);
                    BiomeGenBase[] biomes = replayChunk(cx, cz, getBlendChunk(cx, cz));
                    surfaces = replay.spawnSurfaces(cx, cz, biomes, manager.getBiomesToSpawnIn(), spawnCaves);
                    // Publish only after every stage succeeds. Biome-only previews never fill
                    // this cache with pre-cave surfaces, and failed cave calls remain retryable.
                    spawnSurfaces.put(key, surfaces);
                } catch (ReflectiveOperationException e) { throw reflectionFailure(x, z, e); }
            }
            return surfaces[Math.floorMod(z, 16) * 16 + Math.floorMod(x, 16)];
        }
